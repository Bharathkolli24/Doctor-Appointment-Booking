...
const MyProfile = () => {
  ...
  const updateUserProfileData = async () => {
      try {
        ...
        // We need to make an  API call to update userProfileData
    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      {
        ...
        <div className='mt-10'>
          {
            isEdit
            ? <button className='border border-gray-600 hover:bg-primary hover:text-white transition-all px-8 py-2 rounded-full' onClick={updateUserProfileData}>Save Information</button>
            : <button className='border border-gray-600 hover:bg-primary hover:text-white transition-all px-8 py-2 rounded-full' onClick={()=>setIsEdit(true)}>Edit</button>
          }
        </div>
    </div>
  )
