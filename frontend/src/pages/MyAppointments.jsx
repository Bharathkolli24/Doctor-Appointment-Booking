...
const MyAppointments = () => {
  ...
  const getUserAppointments = async () => {
    try {
      ...
      if (data.success) {
        setAppointments(data.appointments.reverse())  // here reverse() is for 'we will get new Appointments means latest appointments will stay at the top
  ...
  // To make a payment of Appointment Fee
        ...
        <div>
          {appointments.map((item,index)=>(
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
              ...
              <div></div>   {/* In this div we will add structure that will make it responsive  */}
              <div className='flex flex-col gap-2 justify-center'>
                {!item.cancelled && item.payment && !item.isCompleted && <button className='sm:min-w-48 py-2 border rounded text-md text-white bg-green-400'>Paid</button>}
                ...
                {item.isCompleted && <button className='sm:min-w-48 py-2 border border-green-400 rounded text-green-500'>Appointment Completed</button>}
              </div>
            </div>
          ))}
        </div>
